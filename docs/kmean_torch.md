...
You can also [check here](https://raynardj.github.io/p3self/docs/gowalla_preprocess) for a use case on gowalla dataset. You can see the time taken to run an entire epoch is down from 30+ minutes to 38 seconds using by batch technique, and the wall time is down to 3 seconds if we use a NVIDIA GTX 1070
